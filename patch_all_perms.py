...
for perm_file_name in os.listdir(perms_dir):
    if not perm_file_name.endswith(".permissionset-meta.xml"): continue
    
    perm_file = os.path.join(perms_dir, perm_file_name)
    tree = ET.parse(perm_file)
    root = tree.getroot()
    
    # Determine level of access
    # Viewers get read only. Admins get CRUD.
    is_viewer = "Viewer" in perm_file_name
    ...
    for obj in all_objects:
        if obj not in existing_obj_names:
            ...
            allow_create = 'true' if not is_viewer and not obj.endswith("__mdt") else 'false'
            allow_delete = 'true' if not is_viewer and obj.endswith("__c") else 'false'
            allow_edit = 'true' if not is_viewer and obj.endswith("__c") else 'false'
            allow_read = 'true'
            mod_all = 'true' if not is_viewer and obj.endswith("__c") else 'false'
            view_all = 'true' if obj.endswith("__c") else 'false'
            
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}allowCreate').text = allow_create
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}allowDelete').text = allow_delete
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}allowEdit').text = allow_edit
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}allowRead').text = allow_read
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}modifyAllRecords').text = mod_all
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}object').text = obj
            ET.SubElement(obj_perm, '{http://soap.sforce.com/2006/04/metadata}viewAllRecords').text = view_all
            obj_perms.append(obj_perm)
                ...
                for field_file in os.listdir(fields_dir):
                    if field_file.endswith(".field-meta.xml"):
                        ...
                        if not is_req and not is_master and full_field not in existing_field_names:
                            ...
                            ET.SubElement(f_perm, '{http://soap.sforce.com/2006/04/metadata}editable').text = 'true' if not is_viewer else 'false'
    ...
    print(f"Patched {perm_file_name}")
