...
## 7. Dev Hub Config & Accessibility Verdict (71B-12 Update)

*   **CLI Default Dev Hub**: Confirmed that `astrosoft` has been set as the local default Dev Hub:
    ```bash
    sf config set target-dev-hub=astrosoft
    ```
    This is verified by `sf org list` displaying the default Dev Hub marker (`🌳,🍁` indicators) next to the `astrosoft` profile.
*   **Final Access Verdict**: If the Setup page `/lightning/setup/DevHub/home` is still unavailable or denies access after logging in to `vjdev@asap.com`, the current Dev Hub org or user profile lacks Namespace Registry capabilities.
*   **Actionable Fix**: The release owner must either:
    1. Utilize a Partner Business Org (PBO) Dev Hub with proper packaging features enabled.
    2. Request Salesforce Partner Support to link the `sentinelflow` namespace or enable the registry feature in this developer hub org.
*   **Status**: Packaging candidate creation (71C) remains **BLOCKED**. Do not change the `sfdx-project.json` namespace or attempt package version generation.
